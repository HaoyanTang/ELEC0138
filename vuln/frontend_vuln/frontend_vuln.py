...
def get_my_locks(username: str):
    try:
        response = requests.get(
            f"{BACKEND_URL}/locks/me/{username}",
            timeout=5,
        )
        if response.status_code == 200:
            return True, response.json()
...
def pair_lock(username: str, lock_id: int, password: str):
    try:
        response = requests.post(
...
def toggle_lock(username: str, lock_id: int):
    try:
        response = requests.post(
...
def refresh_locks():
    if not st.session_state.username:
        return
    ok, result = get_my_locks(st.session_state.username)
    if ok:
        st.session_state.locks = result
    else:
        st.session_state.locks = []
        set_message(f"unable to get lock: {result}")
...
def screen_login():
    ...
    if login_clicked:
        ...
        if ok:
            st.session_state.token = result.get("access_token", "dummy-token")
...
def screen_pair():
    ...
    if submit_clicked:
        ok, result = pair_lock(st.session_state.username, int(lock_id), pair_password)
        if ok:
            refresh_locks()
    ...
    for lock in locks:
        ...
        with st.container(border=True):
            ...
            if c3.button("Toggle", key=f"toggle_{lock_id}", use_container_width=True):
                ok, result = toggle_lock(st.session_state.username, lock_id)
                if ok:
                    refresh_locks()
