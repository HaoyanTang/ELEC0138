...
async def pair_lock(
    ...
    response = requests.post(headers={"signature": signature}, json=pair_request, url=f"http://127.0.0.1:8001/lock/pair", verify = False)
    try:
...
async def control_lock(
    ...
    response = requests.post(headers={"signature": signature}, json=toggle_request, url=f"http://127.0.0.1:8001/lock/toggle", verify=False)
    try:
        response.raise_for_status()
    except requests.HTTPError:
        raise HTTPException(
